...
-- 1차 카테고리별 통계
SELECT C.CG_NAME, SUM(D.ODR_PRICE)
...
-- 위의 서브쿼리, 셀프조인
SELECT C2.CG_NAME, C1.CG_CODE_C
FROM TBL_CATEGORY C1, TBL_CATEGORY C2
WHERE C1.CG_CODE_P = C2.CG_CODE_C
ORDER BY C1.CG_CODE_C;
...
-- 2치 카테고리 서브쿼리
SELECT C.CG_CODE_C, C.CG_NAME
FROM TBL_CATEGORY C, TBL_PRODUCT P
WHERE C.CG_CODE_C = P.CG_NUM_2
GROUP BY C.CG_CODE_C, C.CG_NAME
ORDER BY C.CG_CODE_C;

-- 월별 주문통계



SELECT * FROM TBL_CATAGORY C1, TBL_CATEGORY C2 WHERE C2.CG_CODE_P = P.CG_NUM_1;
