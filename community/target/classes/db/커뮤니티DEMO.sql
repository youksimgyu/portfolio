...
SELECT BOA_NUM, COUNT(REC_UP) AS REC_UP, COUNT(REC_DOWN) AS REC_DOWN
FROM RECOMMEND
GROUP BY BOA_NUM;
...
FROM (
		...
		SELECT /*+ INDEX_DESC(BOARD PK_BOA_NUM)*/ ROWNUM RN, BOA_NUM, REC_UP, REC_DOWN
		FROM(
	         SELECT BOA_NUM, COUNT(REC_UP) AS REC_UP, COUNT(REC_DOWN) AS REC_DOWN
	         FROM RECOMMEND
	         GROUP BY BOA_NUM
...
SELECT RN, BOA_NUM, REC_UP, REC_DOWN
FROM 
    (SELECT /*+ INDEX_DESC(BOARD PK_BOA_NUM)*/ ROWNUM RN, B.BOA_NUM, R.REC_UP, R.REC_DOWN
...
FROM(	
        SELECT /*+ INDEX_DESC(BOARD PK_BOA_NUM)*/ ROWNUM RN, BOA_NUM, REC_UP, REC_DOWN
        FROM (
                SELECT B.BOA_NUM, R.REC_UP, R.REC_DOWN, B.CAT_P
                 FROM (SELECT BOA_NUM, COUNT(REC_UP) AS REC_UP, COUNT(REC_DOWN) AS REC_DOWN
                       FROM RECOMMEND
                       GROUP BY BOA_NUM) R
                 RIGHT JOIN BOARD B
...
SELECT RN, BOA_NUM, MEM_NAME, CAT_C, CAT_NAME, CAT_NAME_C, CAT_P, BOA_TITLE,
		...
		FROM ( SELECT /*+ INDEX_DESC(BOARD PK_BOA_NUM)*/
                      ...
                      BOA_CONTENT, BOA_DATE_REG, BOA_DATE_UP,
                      EVE_NAME, BOA_HIT, BOA_REP_NUM, REP_UP, REP_DOWN FROM
			    (SELECT B.BOA_NUM, M.MEM_ID, M.MEM_NAME, B.CAT_C, B.CAT_NAME, B.CAT_NAME_C, B.CAT_P, B.BOA_TITLE,
				   	   B.BOA_CONTENT, B.BOA_DATE_REG, B.BOA_DATE_UP,
				   	   B.EVE_NAME, B.BOA_HIT, B.BOA_REP_NUM, B.REP_UP, B.REP_DOWN
			     FROM BOARD B LEFT JOIN MEMBER M
